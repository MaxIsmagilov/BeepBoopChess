...
static inline move_wrapper pull_move(std::string mv, Board* bd)
{
  ...
  if (mv.length() == 5) {
    char pc = mv[4];
    if (pc == 'Q' || pc == 'q') promotion = 4UL;
    if (pc == 'R' || pc == 'R') promotion = 3UL;
    if (pc == 'B' || pc == 'b') promotion = 2UL;
    if (pc == 'K' || pc == 'k') promotion = 1UL;
  }
  for (move_wrapper i : arr) {
    if (start_square(i) == source && end_square(i) == end && promotion == promotion_piece(i)) return i;
  }
  return {0, 0};
}
...
static inline int parse_input(const std::string& input_string)
{
  if (input_string.substr(0, 8) == "position") {
    if (input_string.substr(0, 17) == "position startpos") {
      ...
      if (input_string.size() != 17) {
        ...
        do {
          const int move_end = [&]() -> int {
            return std::find_if(&input_string[move_position],
                                &input_string[move_position] + (input_string.size() - move_position),
      ...
      const int fen_end = [&]() -> int {
        return std::find_if(&input_string[12], &input_string[input_string.length() + 1],
                            [&](const char& c) {
                              has_moves = (c == 'm');
                              return has_moves;
                            }) -
  ...
  else if (input_string.substr(0, 2) == "go")  //"go depth 6" "go binc 123124 winc 21301"
  {
    ...
    if ((value = strstr(input_string.c_str(), "btime")) && MAIN_BOARD.get_side_to_move() == -1)
      pub_time._time = atoi(value + 6);
    if ((value = strstr(input_string.c_str(), "movestogo"))) pub_time._movestogo = atoi(value + 10);
    if ((value = strstr(input_string.c_str(), "movetime"))) pub_time._movetime = atoi(value + 9);
    if ((value = strstr(input_string.c_str(), "depth"))) depth = atoi(value + 6);
  ...
  } else if (input_string.substr(0, 7) == "readyok")
    std::cout << "isready\n";
  else if (input_string.substr(0, 10) == "ucinewgame")
