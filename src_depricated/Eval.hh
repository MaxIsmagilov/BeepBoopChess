...
static constexpr int eg_king_table[64] = {-74, -35, -18, -18, -11, 15, 4,  -17, -12, 17,  14,  17,  17,  38,  23,  11,
...
static constexpr const int* mg_pesto_table[6] = {mg_pawn_table, mg_knight_table, mg_bishop_table,
...
static constexpr int game_phase_inc[12] = {0, 0, 1, 1, 1, 1, 2, 2, 4, 4, 0, 0};

void initialize_evaluation()
{
  int piece, p, square;
  for (p = PAWN, piece = WHITE_PIECE(PAWN); p <= KING; piece += 2, p++) {
  ...
  for (int square = 0; square < 64; square++)
  {
      ...
      for (int piece = 0; piece < 6; piece++)
      {
          ...
          else if (board_copy[piece+6] & bit)
          {
              ...
              game_phase += game_phase_inc[BLACK_PIECE(piece)];
              piece = 6;
          }
      }
  }*/
