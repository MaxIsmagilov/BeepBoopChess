...
Game::Game() : m_table() {}
std::tuple<Move, std::size_t> Game::go_time(double time) {
  auto t = MoveFinder::get_best_move_time(m_board, time, Evaluator::eval, m_table);
  m_board.make_move(std::get<0>(t));
  return t;
}

std::tuple<Move, std::size_t> Game::go_depth(int depth) {
  auto t = MoveFinder::get_best_move(m_board, depth, Evaluator::eval, m_table);
  m_board.make_move(std::get<0>(t));
  return t;
}
